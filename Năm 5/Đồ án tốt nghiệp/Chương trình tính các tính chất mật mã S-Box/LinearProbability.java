import java.io.BufferedWriter;
import java.io.File;
import java.io.FileWriter;
import java.io.IOException;

public class LinearProbabilityV3 {
	public static String checkBin(String s) {
		String result = s;
		while (result.length() < 8) {
			result = "0" + result;
		}
		return result;
	}
	public static int inner_product(int i, int i1) {
		String getBinvalue_i=checkBin(Integer.toBinaryString(i));
		String getBinvalue_i1=checkBin(Integer.toBinaryString(i1));
		int sum=0;
		for (int j = 0; j < 8; j++) {
			sum+=Integer.valueOf(String.valueOf(getBinvalue_i.charAt(j)))*Integer.valueOf(String.valueOf(getBinvalue_i1.charAt(j)));
		}
		return sum%2;
		
	}
	public static void shiftRight(int[] arr) {
		int temp = arr[255];
		for (int i = 254; i >= 0; i--) {
			arr[i + 1] = arr[i];
		}
		arr[0] = temp;
	}
	public static void main(String[] args) throws IOException {
//		File file=new File("C:\\Users\\Admin\\eclipse-workspace\\CryptographyProperties\\src\\Result_MaxLinearProbability.txt");
//		FileWriter fw=new FileWriter(file);
//		BufferedWriter bw=new BufferedWriter(fw);
		int SBox[] = { };
//		int SBox[]= {0x0f, 0x03, 0x0a, 0x06, 0x04, 0x01, 0x0b, 0x09, 0x0e, 0x05, 0x00, 0x0d, 0x02, 0x0c, 0x07, 0x08};
//		for (int m = 1; m < 256; m++) {
//			
//		shiftRight(SBox);
		int max_value=0;
		for (int k = 0; k < 256; k++) { // input mask
			for (int j = 0; j < 256; j++) { // output mask
				int count = 0;
				for (int i = 0; i < 256; i++) {
//					Tính vế phải S(x) AND O_y (O_y là output mask)
					int result_side_right = inner_product(SBox[i], j);
//					Tính vế trái x AND I_y (I_x là input mask)
					int result_side_left = inner_product(i, k);
					if ((result_side_left==result_side_right)) {
						count += 1;
					}
				}
				count=(Math.abs(count-128));
				if(count>max_value&&k!=0&&j!=0) {
					max_value=count;
				}
//				bw.write((count - 128)+" "); // In bảng Linear Probability
//				System.out.println(count);
			}
//			bw.write("\n"); // In bảng Linear Probability
		}
//		bw.write(max_value+"\n");
		
		System.out.println(max_value);
//		}
//		bw.close();
	}
}
