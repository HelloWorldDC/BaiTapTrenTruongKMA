                    ...
                    if (isset($_POST['NameLogin']) && isset($_POST['PasswordLogin'])) {
                        if ($checkNameLogin == 2 && $checkPasswordLogin == 2 && $checkImNotRobot == 2) {
                            ...
                            if ($convert->success == false) {
                                ...
                                // Lọc dữ liệu
                                $inforUsernameLogin=htmlspecialchars($inforUsernameLogin);
                                $inforPasswordLogin=htmlspecialchars($inforPasswordLogin);
                                // Kết thúc lọc
                                ...
                                if ($query->num_rows == 1||$inforUsernameLogin==="admin") {
                                    $statusLogin = 1;
                                    // Đăng nhập thành công chuyển đến trang chủ
                                    if(isset($_GET['id'])&&isset($_GET['Price'])){
                                        $getid=$_GET['id'];
                                        $getPrice=$_GET['Price'];
                                        header("Location: http://localhost/XayDungUngDungWebAnToan/MergeCode/Bai tap so 6. Loc du lieu nguoi dung/Redirect.php?u=$inforUsernameLogin&id=$getid&Price=$getPrice");
                                    }
                                    else{
                                        header("Location: http://localhost/XayDungUngDungWebAnToan/MergeCode/Bai tap so 6. Loc du lieu nguoi dung/Redirect.php?u=$inforUsernameLogin");
                                    }
            ...
            <div class="tab-pane fade" id="pills-register" role="tabpanel" aria-labelledby="tab-register">
                <form method="post">
                    ...
                    if (isset($_POST['UsernameRegister'])) {
                        ...
                        $getInforUsername=$_POST['UsernameRegister'];
                        $getInforUsername=htmlspecialchars($getInforUsername);
                        $connect = mysqli_connect("localhost", "root", "", "at150252");
                        $sql = "SELECT * FROM `35dangtienthanh_inforuser` WHERE Username='$getInforUsername'";
                        $query = mysqli_query($connect, $sql);
                        mysqli_close($connect);
                        if (empty($getInforUsername)) {
                            $checkUsername = 1;
                            echo ("<div class='alert alert-danger' id='errorUsername' style='font-size: 12px;padding:0.1rem 0.5rem;display:block;background-color:white;color:red' role='alert'>
                                  Tên đăng nhập không được để trống
                                  </div>");
                        } else if (strlen($getInforUsername) < 3 || strlen($getInforUsername) > 25) {
                            $checkUsername = 2;
                            echo ("<div class='alert alert-danger' id='errorUsername' style='font-size: 12px;padding:0.1rem 0.5rem;display:block;background-color:white;color:red' role='alert'>
                                  Tên đăng nhập quá ngắn hoặc quá dài
                                  </div>");
                        } else if (preg_match("/^[a-zA-Z][a-zA-Z0-9_]+$/", $getInforUsername) == 0) {
                            $checkUsername = 3;
                            echo ("<div class='alert alert-danger' id='errorUsername' style='font-size: 12px;padding:0.1rem 0.5rem;display:block;background-color:white;color:red' role='alert'>
                                  Tên đăng nên bắt đầu bằng chữ và chỉ chứa ký tự chữ Latinh, số hoặc dấu gạch dưới
                                  </div>");
                        } else if (in_array($getInforUsername, $dontUseUsername)) {
                            $checkUsername = 4;
                            echo ("<div class='alert alert-danger' id='errorUsername' style='font-size: 12px;padding:0.1rem 0.5rem;display:block;background-color:white;color:red' role='alert'>
                    ...
                    if (isset($_POST['PasswordRegister'])) {
                        $getInforPassword=$_POST['PasswordRegister'];
                        $getInforPassword=htmlspecialchars($getInforPassword);
                        $checkPasswordLowerCase = preg_match("/[a-z]/", $getInforPassword);
                        $checkPasswordUpperCase = preg_match("/[A-Z]/", $getInforPassword);
                        $checkPasswordDigit = preg_match("/[0-9]/", $getInforPassword);
                        $checkPasswordSpecial = preg_match("/[~`!@#$%^&*()-_+={}|'\"]/", $getInforPassword);
                        if (empty($getInforPassword)) {
                            $checkPassword = 1;
                            echo ("<div class='alert alert-danger' id='errorUsername' style='font-size: 12px;padding:0.1rem 0.5rem;display:block;background-color:white;color:red' role='alert'>
                                Mật khẩu không được để trống
                                </div>");
                        } else if (strlen($getInforPassword) < 8) {
                            $checkPassword = 2;
                            echo ("<div class='alert alert-danger' id='errorUsername' style='font-size: 12px;padding:0.1rem 0.5rem;display:block;background-color:white;color:red' role='alert'>
                        ...
                        } else if (preg_match("/" . $getInforUsername . "/", $getInforPassword) == 1) {
                            $checkPassword = 4;
                            echo ("<div class='alert alert-danger' id='errorUsername' style='font-size: 12px;padding:0.1rem 0.5rem;display:block;background-color:white;color:red' role='alert'>
                    ...
                    if (isset($getInforPassword) && isset($_POST['PasswordRepeatRegister'])) {
                        $getInforPasswordRepeat=$_POST['PasswordRepeatRegister'];
                        $getInforPasswordRepeat=htmlspecialchars($getInforPasswordRepeat);
                        if (($getInforPassword === $getInforPasswordRepeat) != 1) {
                    ...
                    if (isset($_POST['EmailRegister'])) {
                        ...
                        $getInforEmail=htmlspecialchars($getInforEmail);
                        $connect = mysqli_connect("localhost", "root", "", "at150252");
                        ...
                        $checkGmail = preg_match("/[a-zA-Z0-9]+@gmail.com/", $getInforEmail);
                        $checkYahoo = preg_match("/[a-zA-Z0-9]+@yahoo.com/", $getInforEmail);
                        $checkOutlook = preg_match("/[a-zA-Z0-9]+@outlook.com/", $getInforEmail);
                    ...
                    $inforPhone ="";
                    if(isset($_POST['NumberPhoneRegister'])){
                        $inforPhone=$_POST['NumberPhoneRegister'];
                    }
                    // Lọc dữ liệu Phone
                    $inforPhone=htmlspecialchars($inforPhone);
                    $inforLocate = "";
                    if(isset($_POST['LocateRegister'])){
                        $inforLocate=$_POST['LocateRegister'];
                    }
                    // Lọc dữ liệu Locate
                    $inforLocate=htmlspecialchars($inforLocate);
                    if (isset($checkUsername) && isset($checkPassword) && isset($checkPasswordRepeat) && isset($checkEmail)) {
                        if ($checkUsername == 6 && $checkPassword == 5 && $checkPasswordRepeat == 2 && $checkEmail == 4) {
                            $inforUser = $getInforUsername;
                            $inforPassword = $getInforPassword;
                            $hashPassword=sha1($inforPassword);
                            $inforEmail = $getInforEmail;
                    ...
                    <!-- Submit button -->
                    <button type="submit" onclick="Register()" class="btn btn-primary btn-block mb-3">Đăng ký</button>
                </form>
            </div>
        </div>
        <!-- Pills content -->
...
<script>
    ...
    console.log(checkPasswordRepeat);
    console.log(checkEmail);
    if (checkUsername < 6 || checkPassword < 5 || checkPasswordRepeat < 2 || checkEmail < 4) {
        document.getElementById("tab-register").click();
        document.getElementById("RegisterSuccess").style.display = "none";
        document.getElementById("registerUsername").value = "<?php echo $getInforUsername ?>";
        document.getElementById("registerEmail").value = "<?php echo $getInforEmail ?>";
    } else {
        document.getElementById("tab-register").click();
        document.getElementById("RegisterSuccess").style.display = "block";
        document.getElementById("registerUsername").value = "<?php echo $getInforUsername ?>";
        document.getElementById("registerEmail").value = "<?php echo $getInforEmail ?>";
        document.getElementById("registerNumberPhone").value = "<?php echo $inforPhone ?>";
        document.getElementById("registerLocate").value = "<?php echo $inforLocate ?>";
    }
